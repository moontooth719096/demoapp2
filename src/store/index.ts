...
export default createStore<statetype>({
  ...
  mutations: {
    ...
    setNowtalkInfo(state, payload: NowTalkInfo) {
      if (state.chatRoomInfo) {
        ...
        const talk = state.chatRoomInfo.talklist?.find(
          (x) => x.talkid == payload?.UserID
        ) as TalkInfo;
        if (talk) {
    ...
    pushTalk(state, payload: SingleTalkInfo) {
      if (state.chatRoomInfo?.talklist) {
        state.chatRoomInfo.talklist
          .find((x) => x.talkid == payload.talkid)
          ?.talks.push(payload);
      }
    },
  ...
  actions: {
    ...
    refreshChatlist({ commit }, onlineusers: Array<ChatInfo>) {
      ...
      if (this.state.chatRoomInfo) {
        let nowchatlist = onlineusers.filter(
          (x) => x.UserID !== this.state.userInfo.UserID
        );
    ...
    UserDisconnected({ commit }, offlineuserid: ChatInfo) {
      if (this.state.chatRoomInfo) {
        let currentchatRoomInfo = this.state.chatRoomInfo
          .chatlist as Array<ChatInfo>;
        if (currentchatRoomInfo) {
          const index = currentchatRoomInfo.findIndex(
            (item) => item.UserID === offlineuserid.UserID
          );
          if (index) {
    ...
    async talkselect({ commit, dispatch }, selectid: string) {
      ...
      while (
        !this.state.chatRoomInfo.chatlist?.some((x) => x.UserID == selectid)
      ) {
      ...
      //如果有確定有人，抓出那個人的資料
      const user = this.state.chatRoomInfo.chatlist?.find(
        (x) => x.UserID == selectid
      ) as ChatInfo;
      if (user?.NoReadCount > 0) user.NoReadCount = 0;
    ...
    async PrivateMessage({ commit, dispatch }, messageinfo: SingleTalkInfo) {
      ...
      let istry = false as boolean;
      while (
        !this.state.chatRoomInfo.chatlist?.some(
          (x) => x.UserID == messageinfo.talkid
        )
      ) {
        await GetConnectedUsers();
        //判斷確定沒人，不處理
    ...
    async AddTalk({ commit }, messageinfo: SingleTalkInfo) {
      ...
      if (talk) {
        ...
        if (messageinfo.message) {
          newtalks = [
            { sayid: messageinfo.sayid, message: messageinfo.message },
          ];
        }
        const talk: TalkInfo = {
          talkid: messageinfo.talkid ?? "",
          talkImgPath: user.PicturesPath ?? "",
          talks: newtalks,
        };
    ...
    RefreshChat({ commit }) {
      ...
      let nowtalks = this.state.chatRoomInfo.talklist?.find(
        (x) => x.talkid == this.state.chatRoomInfo.nowTalkinfo?.UserID
      );
